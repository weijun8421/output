<#
.SYNOPSIS
    PXE 部署后关键驱动状态检测脚本 v2.0
.DESCRIPTION
    与 v1.0 的全量扫描不同，v2.0 默认只检测影响系统正常使用的核心驱动：
      显卡 (Display) / 网卡 (Net) / 声卡 (Media) / 存储控制器 (HDC, SCSIAdapter)
      USB 控制器 (USB) / 主板芯片组 (System 类关键词筛选)
...
.PARAMETER FullScan
    切换为 v1.0 的全量扫描模式，检测所有 PnP 设备。
.PARAMETER ExtraClass
    在默认关键类别之外追加额外的设备类（逗号分隔）。
    例如 -ExtraClass "Bluetooth,Printer"
.EXAMPLE
    .\driver_check_v2.ps1
    快速模式：检测显卡、网卡、声卡、存储控制器、USB、主板芯片组。
.EXAMPLE
    .\driver_check_v2.ps1 -FullScan
    全量模式：等同于 v1.0，扫描所有 PnP 设备。
.EXAMPLE
    .\driver_check_v2.ps1 -ExtraClass "Bluetooth,Printer"
    在默认关键类别基础上追加蓝牙和打印机。
.NOTES
...
if ($FullScan) {
    ...
    # 追加用户指定的额外类
    if ($ExtraClass) {
        $ExtraList = $ExtraClass -split ',' | ForEach-Object { $_.Trim() } | Where-Object { $_ }
        $TargetClasses += $ExtraList
    }
    # System 类中的主板芯片组关键词（过滤掉风扇/按钮/电池等无关项）
    $SystemKeywordFilter = @(
...
# 2. 采集设备信息
# ============================================================
Write-Host "扫描模式: $ScanMode" -ForegroundColor Cyan
Write-Host "正在扫描 PnP 设备..." -ForegroundColor Cyan
...
function Test-IsTargetDevice {
    ...
    # 固定关键类：直接保留
    if ($class -and $TargetClasses -contains $class) {
        return $true
    }

    # System 类：按关键词筛选主板芯片组相关
    # 也检查 WMI 中的 ClassGuid/PNPClass 以兼容某些设备
    $wmiClass = if ($WmiDetail -and $WmiDetail.PNPClass) { $WmiDetail.PNPClass } else { "" }
    if ($class -eq "System" -or $wmiClass -eq "System") {
        $searchText = if ($name) { $name } else { "" }
        foreach ($kw in $SystemKeywordFilter) {
            if ($searchText -match $kw) { return $true }
        }
        return $false
    }

    # ExtraClass 指定的额外类别
    if ($ExtraClass -and $class) {
        $extraArr = $ExtraClass -split ',' | ForEach-Object { $_.Trim() } | Where-Object { $_ }
        if ($extraArr -contains $class) { return $true }
    }
...
foreach ($dev in $AllDevices) {
    ...
    # v2.0 筛选
    if (-not (Test-IsTargetDevice -PnpDevice $dev -WmiDetail $wmiDetail)) {
        continue
    }
...
$Skipped = $TotalScanned - $TotalCount
Write-Host "扫描完成：全机 $TotalScanned 个设备，过滤后检测 $TotalCount 个关键设备 (跳过 $Skipped 个)，耗时 $([math]::Round($Elapsed, 1)) 秒" -ForegroundColor Green
...
if ($Format -in @("Summary", "All")) {
    $SummaryPath = Join-Path $OutputDir "DriverCheck_v2_${Hostname}_${Timestamp}.txt"

    $SummaryLines = @()
    $SummaryLines += "=" * 70
    $SummaryLines += "     PXE 部署后关键驱动状态检测报告 v2.0"
    $SummaryLines += "=" * 70
    $SummaryLines += "  主机名:     $Hostname"
    $SummaryLines += "  检测时间:   $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
    $SummaryLines += "  扫描模式:   $ScanMode"
    ...
    $SummaryLines += "-" * 70
    $SummaryLines += "  全机设备总数:       $TotalScanned"
    if (-not $FullScan) {
        $SummaryLines += "  本次检测(关键设备): $TotalCount"
    }
    $SummaryLines += "  正常设备:           $OkCount"
    $SummaryLines += "  ─────────────────────────────────"
    $SummaryLines += "  驱动缺失/异常:      $MissingCount"
    $SummaryLines += "  设备已禁用:         $DisabledCount"
    $SummaryLines += "  等待重启生效:       $PendingCount"
    $SummaryLines += "  正在移除:           $RemovingCount"
    ...
    if ($OkDevices.Count -gt 0) {
        $SummaryLines += ""
        $SummaryLines += ">>> 已安装驱动清单 (共 $($OkDevices.Count) 个) <<<"
        $SummaryLines += ""
    ...
    if ($ProblemDevices.Count -gt 0) {
        ...
        $SummaryLines += ""
        $SummaryLines += "  [OK] 所有关键设备驱动均已正常安装。"
    }

    $SummaryLines += "=" * 70
    $SummaryLines += "  报告结束 | 驱动检测脚本 v2.0"
    $SummaryLines += "=" * 70
...
if ($Format -in @("CSV", "All")) {
    $CsvPath = Join-Path $OutputDir "DriverCheck_v2_${Hostname}_${Timestamp}.csv"
...
# ============================================================
Write-Host ""
Write-Host "============ 关键驱动检测结果 ============" -ForegroundColor Cyan
Write-Host "  扫描: $ScanMode" -ForegroundColor White
Write-Host "  检测: $TotalCount 个关键设备 | 正常: $OkCount | 问题: $($ProblemDevices.Count)" -ForegroundColor White
...
if ($MissingCount -gt 0) {
    Write-Host ""
    Write-Host "驱动缺失/异常的设备:" -ForegroundColor Red
    $MissingList = $ResultList | Where-Object { $_.Category -eq "MISSING" }
    foreach ($m in $MissingList) {
        $name = if ($m.FriendlyName) { $m.FriendlyName } else { "(未知设备)" }
        Write-Host "  - $name   ErrorCode=$($m.ErrorCode) | $($m.ErrorDesc)" -ForegroundColor White
        if ($m.HardwareID) {
...
# 退出码
if ($ProblemDevices.Count -gt 0) { exit 1 } else { exit 0 }
